...
Копию стоит делать перед любой разрушительной операцией: очисткой базы или
ручными правками.

`db:restore` — пара к `db:reset`. На непустой базе отказывается работать:
восстановление поверх смешало бы два состояния. Порядок такой — `db:backup`,
`db:reset -- --yes`, `db:restore`. Заливка идёт пакетами в одной транзакции:
по записи за раз транзакция не доживала до конца на четырёх сотнях трат.
...
таблицы, а наполняет их только восстановление из копии.
