...
4. **Тело запроса приходится подкладывать обратно.** Платформа разбирает
   его сама и кладёт в `req.body`, а исходный поток оставляет пустым.
   Hono читает тело из потока и ждёт данных, которых не будет, — POST и PUT
   висят до `FUNCTION_INVOCATION_TIMEOUT`, при том что GET работают.
5. **Default-экспорт — обработчик в стиле Node `(req, res)`.** Рантайм зовёт
...
Logs. Нарушения 4 и 5 — это `FUNCTION_INVOCATION_TIMEOUT`, и в логах при них
не будет ничего: код отработал, просто ответ не записан. Нарушение 4 при этом
коварнее остальных: чтение работает, ломается только запись.
...
Проверка транспилирует дерево как платформа и проверяет все пять условий,
включая запрос **с телом** в обоих режимах доставки — когда тело ещё в потоке
и когда платформа прочитала его заранее.
...
### Резервная копия, очистка, восстановление

```bash
npm run db:backup -- ../backup.json      # выгрузить текущее состояние
npm run db:reset -- --yes                # стереть всё (требует подтверждения)
npm run db:restore -- ../backup.json     # залить копию обратно
```

Копию стоит делать перед любым повторным импортом: он перезаписывает
записи по идентификаторам, и правки, сделанные в приложении, теряются.

`db:restore` — пара к `db:reset`: он читает формат `db:backup`, а не выгрузку
из веба (для неё `import:legacy`). На непустой базе отказывается работать:
восстановление поверх смешало бы два состояния. Порядок такой — `db:backup`,
`db:reset -- --yes`, `db:restore`. Всё заливается одной транзакцией, половины
восстановленной базы не бывает.

### Исключённые записи

`EXCLUDED_EXPENSES` (`src/server/lib/legacy.ts`) пуст намеренно: выгрузка
переносится один в один. Записи, которые выглядели обходными приёмами веба,
оказались настоящими деньгами — исключать их значило бы завышать остаток.

Список оставлен как точка расширения: если найдётся запись, которую
действительно нельзя переносить, её добавляют туда с указанием причины,
а не вычищают из файла выгрузки. Выгрузку можно пересобрать запросом,
и правка руками при этом потерялась бы.

Единственное, что добавляется к данным, — `ENDED_AT`: даты завершения работ.
В старой схеме у оклада не было даты окончания, и вывести её из данных нельзя.
