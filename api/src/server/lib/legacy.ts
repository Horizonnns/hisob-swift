...
/**
 * Записи, которые не переносятся, и причина по каждой.
 *
 * В веб-версии перенос остатка считался внутри проекта и обрывался, поэтому
 * приходилось заводить искусственные траты, чтобы обнулить остаток вручную.
 * В новой модели цепочка сквозная и считается сама — такие записи стали бы
 * настоящим расходом и исказили бы и месяц, и весь перенос дальше.
 *
 * Ключ — `id` из старой базы. Исключение объявлено здесь, а не вычищено из
 * файла выгрузки: выгрузку можно пересобрать запросом заново, и правка руками
 * при этом потерялась бы.
 */
export const EXCLUDED_EXPENSES: Record<string, string> = {
	cmsumhcu00135l3040xdv8bbq:
		'Искусственная запись на 9 980 TJS от 2026-06-01 («ТРАТА ЗА СЧЕТ ЗП Май месяца»): ' +
		'заводилась, чтобы обнулить остаток при неработающем переносе.'
}

export function exclusionReason(legacyId: string): string | null {
	return EXCLUDED_EXPENSES[legacyId] ?? null
}
